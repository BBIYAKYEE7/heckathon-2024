...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            cv2.circle(img, (nose_x, nose_y), 6, (0, 0, 255), cv2.FILLED)

            if not baseline_set:
                baseline_length = nose_y
                baseline_set = True

            if abs(nose_y - baseline_length) > scroll_threshold * baseline_length:
                pyautogui.scroll(int((nose_y - baseline_length) * scroll_speed))
